...
creds = None
if os.path.exists('/home/ubuntu/helixBot/token.pickle'):
    with open('/home/ubuntu/helixBot/token.pickle', 'rb') as token:
...
if not creds or not creds.valid:
    if creds and creds.expired and creds.refresh_token:
        ...
        flow = InstalledAppFlow.from_client_secrets_file(
            '/home/ubuntu/helixBot/credentials.json', SCOPES)
        creds = flow.run_local_server(port=0)
    # Save the credentials for the next run
    with open('/home/ubuntu/helixBot/token.pickle', 'wb') as token:
